...
```
请阅读项目中 CanReceiveCoroutine.kt 和 DB9reCANseCoroutine.kt 的代码，
理解现有CAN串口的帧格式和收发方式。

然后创建 funClass/CanOpenFun.kt，实现 CANopen 协议的底层通信：
1. SDO 写入帧构建（支持1/2/4字节，参考手册SDO格式）
2. SDO 读取帧构建
3. SDO 回复解析
4. 便捷方法：使能/位置模式/速度模式/绝对位置运动/速度运动/急停/读位置/读状态
5. NMT 启动命令
6. 帧封装方法 wrapCanFrame() 要与现有CAN串口格式一致

CANopen协议参考 SOWING_DEPTH_IMPLEMENTATION.md 第二节。
CAN-ID: SDO发送 0x600+ID, SDO接收 0x580+ID
数据小端序。
```
...
```
1. 创建 data/SowingDepthData.kt，包含 ServoCalibration 和 SowingDepthState 数据类
2. 在 MySharedPreFun.kt 中新增播种深度参数的保存/读取方法：
   - 8个电机的 Node-ID、限位值、校准点、拟合系数
   - 全局点动速度、位置运动速度
3. 在 ViewModelAndPublic.kt 中的 VariableFertViewModel 里新增播种深度状态

参考 SOWING_DEPTH_IMPLEMENTATION.md 4.4节的数据结构设计。
```

### 阶段3: CAN接收扩展

```
修改 CanReceiveCoroutine.kt：
在CAN接收循环中，增加对 CANopen 帧的识别：
- CAN-ID 范围 0x581~0x5FF 是 SDO 回复（0x580+NodeID）
- CAN-ID 范围 0x181~0x1FF 是 TPDO1（0x180+NodeID）
- CAN-ID 范围 0x701~0x77F 是心跳（0x700+NodeID）

解析后更新 ViewModel 中对应伺服电机的状态（位置、状态字、报警码）。
不要破坏现有施肥电机的CAN消息处理逻辑。
```

### 阶段4: 播种深度业务逻辑

```
创建 funClass/SowingDepthFun.kt：
1. calculateCalibrationPositions(limitMin, limitMax) → 5个等分位置
2. linearFit(points) → (a, b) 最小二乘法
3. depthToPosition(depthMm, a, b) → 编码器位置
4. positionToDepth(position, a, b) → 深度mm
5. isPositionSafe(position, limitMin, limitMax) → 安全检查

参考 SOWING_DEPTH_IMPLEMENTATION.md 4.5节。
```

### 阶段5: 控制协程

```
创建 coroutine/SowingDepthCoroutine.kt：
参考 DB9reCANseCoroutine.kt 的协程模式。

主循环（500ms间隔）：
1. 遍历8个伺服电机
2. 对已使能的电机：发送读取位置命令，更新当前位置到ViewModel
3. 如果目标深度发生变化：通过拟合曲线计算目标编码器位置，做安全检查，发送绝对位置命令
4. 检查报警状态
5. 心跳维持（每次循环读取就是CAN通信，足以防止2秒超时）
...
```
创建 ui/DepthCalibrationScreen.kt，使用 Jetpack Compose：

界面分两步骤：

步骤1 - 限位设置：
- 显示当前编码器位置（实时更新）
- "Deep"/"Shallow" 点动按钮（按住运动，松开停止，使用速度模式）
- 可调节的点动速度
- "设为最深限位"/"设为最浅限位" 按钮
- 限位设置完成后显示确认，并将限位值写入电机（SDO写0x261F和0x2620）

步骤2 - 5挡位校准（限位设置完成后才可操作）：
- 自动计算5个等分位置
- 每个挡位有 [移动到此位置] 按钮（使用绝对位置模式）
- 每个挡位有深度输入框（用户手动测量填入）
- [计算拟合] 按钮，显示 y=ax+b 结果
- [保存校准] 保存到 SharedPreferences

参考现有 SettingsScreen.kt 的 Compose 写法和 ComposeTemplates.kt 的可复用组件。
```

### 阶段7: 播种深度控制主界面

```
创建 ui/SowingDepthScreen.kt：
...
- 中间：8行电机状态卡片，每行显示：
  ...
  - 当前深度 / 目标深度
  - [单独设置] 可为单个电机设不同深度
- 底部：[标定设置] 导航到校准界面、[全部停止] 急停

在 MainActivity.kt 中添加导航路由。
参考 MainScreen.kt 的布局风格。
```
